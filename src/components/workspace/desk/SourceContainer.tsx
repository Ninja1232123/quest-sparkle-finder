    ...
    >
      <header className="flex shrink-0 items-center justify-between gap-2 border-b px-3 py-2" style={{ borderColor: "var(--rule-card)" }}>
        <div className="min-w-0">
          ...
          <div className="text-[12px] uppercase tracking-[0.16em]" style={{ color: "var(--ink-muted)", fontFamily: "var(--font-mono)" }}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        {state.loading && (
          <div className="flex items-center gap-2 px-1 py-3 text-[12px]" style={{ color: "var(--ink-muted)" }}>
            <Loader2 className="h-3.5 w-3.5 animate-spin" /> Deep-diving the corpus…
          </div>
        )}
        {!state.loading && state.error && (
          <div className="rounded border border-destructive/30 bg-destructive/10 p-2 text-[12px] text-destructive">{state.error}</div>
        )}
        {!state.loading && !state.error && (state.hits === null || count === 0) && (
          <button
            type="button"
            onClick={onRunDeepDive}
            className="flex w-full items-center justify-center gap-1.5 rounded-md border border-dashed px-2 py-4 text-[12px] transition-colors hover:bg-foreground/5"
          ...
          <ul className="space-y-1.5">
            {state.hits.slice(0, 5).map((h) => (
              <li key={h.identifier}>
                ...
                >
                  <div className="truncate text-[12px] font-medium" style={{ color: "var(--ink)", fontFamily: "var(--font-serif)" }}>
                    {h.heading || h.identifier}
                  </div>
                  {h.sectionLabel && (
                    <div className="truncate text-[12px] uppercase tracking-[0.12em]" style={{ color: "var(--ink-muted)", fontFamily: "var(--font-mono)" }}>
                      {h.sectionLabel}
                    </div>
                  )}
                </button>
            ...
            {count > 5 && (
              <li>
                <button type="button" onClick={onExpand} className="w-full px-2 py-1 text-center text-[12px] underline decoration-dotted underline-offset-2" style={{ color: "var(--ink-muted)" }}>
