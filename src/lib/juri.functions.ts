...
- search_cases: search court opinions (CourtListener) for cases that applied a statute or decided a legal question. Use when the user asks how courts have ruled, wants examples of wins/losses, or needs case law to back a position. Returns case names, courts, years, citation counts, and sometimes text snippets.
  HOW CASE SEARCH ACTUALLY WORKS — know these or you'll waste it:
  1. Snippets often come back EMPTY. That's normal — snippets only generate on exact keyword matches in the search index. An empty snippet doesn't mean the case is off-topic; it just means the full text wasn't hit. Don't skip a case because its snippet is blank.
  2. Results rank by citation count + keyword match. High-cited cases float to the top even if only loosely related. A search for "quasi-estoppel" will pull heavily-cited cases that mentioned it once, not necessarily the defining cases. To verify a case is actually on-point, READ it with read_case using the cluster_id.
  3. TO GET ACTUAL CASE CONTENT: use read_case(cluster_id) on any case that looks relevant. That's the only way to confirm what the court actually held. Don't cite a case or summarize its holding from the search result alone — always read it first.
  4. BETTER QUERIES: include the specific legal term OR doctrine AND the factual/statutory context together — e.g. "quasi-estoppel insurance coverage" or "15 USC 1692g bona fide error" beats just the doctrine name. Narrow by statute when you have one. If the first search returns generic high-citation cases, try again with more specific terms or a different angle.
  5. CHAIN THE TOOLS: search_cases → read_case (on promising results) → cite what you read. The search finds candidates; read_case is where you actually learn what was decided.
